    ...
    def display_traffic(road: Road, surface: WorldSurface, simulation_frequency: int = 15, offscreen: bool = False) \
        ...
        if road.record_history:
            for i in range(len(road.vehicles)):
                VehicleGraphics.display_history(road.vehicles[i], surface, simulation=simulation_frequency, offscreen=offscreen)
        for i in range(len(road.vehicles)):
            VehicleGraphics.display(road.vehicles[i],i, surface, offscreen=offscreen)
